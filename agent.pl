...
% if head of Events is truffle(X, Y, S), add it at head of Goals_truff.
trigger([truffle(X, Y, S)|Events], goals(Goals_rest, [goal(X, Y, S)|Goals_truff])) :-
    trigger(Events, goals(Goals_rest, Goals_truff)).

% if head of Events is restaurant(X, Y, S), add it at head of Goals_rest.
trigger([restaurant(X,Y,S)|Events], goals([goal(X,Y,S)|Goals_rest], Goals_truff)) :-
    trigger(Events, goals(Goals_rest, Goals_truff)).
...
%
% check if the number of truffles in the agent's stock is greater than
...
% Observation is at (X, Y), update at(X, Y) in Beliefs.
update_beliefs(at(X, Y), beliefs(_, stock(T)), beliefs(at(X, Y), stock(T))).

% Observation is picked(X, Y, S), update stock(T1) in Beliefs. T1 = T + S
update_beliefs(picked(X, Y, S), beliefs(_, stock(T)), beliefs(at(X, Y), stock(T1))) :-
    T1 is T + S.
% bservation is sold(X, Y, S), update stock(T1) in Beliefs. T1 = T - S
